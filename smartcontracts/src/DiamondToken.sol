...
/**
 * @title DiamondToken
 * @dev Diamond (DMD) - Stable trading pair token for KASI AMM
 *
 * Features:
 * - Standard ERC-20 functionality
 * - Mintable by owner for liquidity provision
 * - Burnable for deflationary mechanics
 * - Pausable for emergency situations
 * - Used as base pair in Faucet AMM (DIAMOND/KASI)
 */
contract DiamondToken is ERC20, ERC20Burnable, Ownable, Pausable {
     ...
     */
    constructor(address initialOwner) ERC20("Diamond", "DMD") Ownable(initialOwner) {
        require(initialOwner != address(0), "DiamondToken: Invalid owner address");
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(to != address(0), "DiamondToken: Cannot mint to zero address");
        require(totalSupply() + amount <= MAX_SUPPLY, "DiamondToken: Would exceed max supply");
    ...
    function _update(address from, address to, uint256 value) internal virtual override {
        ...
        require(!paused(), "DiamondToken: Token transfers are paused");
    }
